...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            #Ypred - prediction for only one point
            #Y - prediction for all points to be processed
            #print('X[;n]=',X[:,n][:,np.newaxis])
            Ypred = self.forward(X[:,n][:,np.newaxis])
            #print('Ypred=',Ypred)
            #print('Y=',Y[:,n])
            #print('loss=',self.loss.forward(Ypred,Y[:,n].reshape(Ypred.shape)))
            self.loss.forward(Ypred,Y[:,n].reshape(Ypred.shape))
