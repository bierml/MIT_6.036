...
from tensorflow.keras.layers import Conv1D, Conv2D, Dense, Dropout, Flatten, MaxPooling2D,MaxPooling1D,Activation
...
def run_keras(X_train, y_train, X_val, y_val, X_test, y_test, layers, epochs, split=0, verbose=True):
    ...
    if X_val is not None or split > 0:
        ...
        #THE SECOND VER
        val_acc, val_loss = model.history.history['val_accuracy'][-1], history.values['epoch_val_loss'][-1]
        history.values['epoch_val_acc'] = model.history.history['val_accuracy']
        history.values['epoch_acc'] = model.history.history['accuracy']
        #THE THIRD VER
        #val_acc, val_loss = model.history.history['val_accuracy'][-1], model.history.history['epoch_val_loss'][-1]
        #THE SECOND WAY
        #val_acc = model.history.history['val_accuracy']
        #OLDVER
        print ("\nLoss on validation set:"  + str(val_loss) + " Accuracy on validation set: " + str(val_acc))
        #NEWVER
        #print ("\nLoss on validation set:"  + str(val_loss) + " Accuracy on validation set: " + str(model.history.history['val_accuracy']))
...
def run_keras_2d(data_name, layers, epochs, display=True, split=0.25, verbose=True, trials=1):
    ...
    y_train = utils.to_categorical(y, num_classes) # one-hot
    y_val = y_test = None
    if X_val is not None:
        #y_val = np_utils.to_categorical(y2, num_classes) # one-hot 
        y_val = utils.to_categorical(y2, num_classes) # one-hot     
    if X_test is not None:
    ...
    for trial in range(trials):
        ...
        if display:
            ...
            # Plot epoch accuracy
            #CONTINUE HERE
            #EPOCH ACC AND AND EPOCH VAL ACC ARE BOTH EMPTY
            #SHOULD THINK ABOUT WHY THEY HAVE NOT VALUES
...
def l1_reg(weight_matrix):
    return 1 * K.sum(K.abs(weight_matrix))    

class MyRegularizer(regularizers.Regularizer):
    def __init__(self, lam=1):
        self.lam = lam
    def __call__(self, weights):
        return self.lam*K.abs(K.sum(weights))
    def get_config(self):
...
def run_keras_fc_mnist(train, test, layers, epochs, split=0.1, verbose=True, trials=1):
    ...
    # Categorize the labels
    num_classes = 10
    y_train = utils.to_categorical(y1, num_classes)
    y_val = utils.to_categorical(y2, num_classes)
...
#run_keras_2d("3class", archs(3)[0], epochs=10, split=0.5, display=False, verbose=False, trials=1)
#CONTINUE WITH FIXING THIS CODE
#layers=[Conv1D(filters=1, kernel_size=2, strides=1,use_bias=False, activation='relu', input_shape=(128,1), padding='same',kernel_regularizer=None),MaxPooling1D(pool_size=2,strides=2, padding="same"),Flatten(),Dense(units=1, activation=None, use_bias=False)]
#print(train_neural_counter(layers,get_image_data_1d(1000,(128,1),0.1),loss_func='mse',display=True))
#layers=[Conv1D(filters=1, kernel_size=2, strides=1,use_bias=False, activation='relu', input_shape=(128,1), padding='same',kernel_regularizer=None),Flatten(),Dense(units=1, activation=None, use_bias=False)]
#print(train_neural_counter(layers,get_image_data_1d(1000,(128,1),0.1),loss_func='mse',display=True))
...
run_keras_fc_mnist(train, validation, [
    Dense(input_dim=48*48, units=512, activation="relu"), Dense(units=256, activation="relu"),Dense(units=10, activation="softmax")],1, split=0.1, verbose=False, trials=5)
