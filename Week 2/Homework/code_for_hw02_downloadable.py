...
def perceptron(data, labels, params={}, hook=None):
    ...
    th0 = np.zeros((1,1))
    #print((th.T @ (data[:,0].reshape((data.shape[0],1)))).shape)
    #print(th.T.shape)
    #print(labels[:,0].reshape(1,1))
    for t in range(T):
        for i in range(data.shape[1]):
            #print(labels[:,i].reshape(1,1)*(th.T@(data[:,i].reshape((data.shape[0],1)))+th0))
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    for t in range(T):
        for i in range(data.shape[1]):
            ...
            th0s = th0s + th0
    if hook!=None:
        hook((ths/(T*data.shape[1]),th0s/(T*data.shape[1])))
    return (ths/(T*data.shape[1]),th0s/(T*data.shape[1]))
    pass
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    return score(data_test,labels_test,learner(data_train,labels_train)[0],learner(data_train,labels_train)[1])/labels_test.shape[1]
    pass
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    res=0
    for i in range(it):
        training_data=data_gen(n_train)
        testing_data=data_gen(n_test)
        #res = res + eval_classifier(learner,training_data[0],training_data[1],testing_data[0],testing_data[1])
        res = res + eval_classifier(learner,training_data[0],training_data[1],training_data[0],training_data[1])
    res = res / it
    return res
    pass
...
def xval_learning_alg(learner, data, labels, k):
    score = 0
    datasplitting = np.array(np.array_split(data,k,axis=1))
    #print(data == np.concatenate(datasplitting,axis=1))
    labelssplitting = np.array(np.array_split(labels,k,axis=1))
    #print(labels == np.concatenate(labelssplitting,axis=1))
    #print(data.shape)
    #print(labels.shape)
    #print(k)
    #print(datasplitting.shape)
    #print(labelssplitting.shape)
    #print(datasplitting[0:2])
    #print('------')
    #print(datasplitting[0].shape)
    #print('------\n')
    #print(datasplitting[2:3])
    #cross validation of learning algorithm
    for j in range(k):
        D_minus_j = np.concatenate(np.delete(datasplitting,j,axis=0),axis=1)
        L_minus_j = np.concatenate(np.delete(labelssplitting,j,axis=0),axis=1)
        score = score + eval_classifier(learner,D_minus_j,L_minus_j,datasplitting[j],labelssplitting[j])
    '''for j in range(k+1):
       #print(datasplitting[j:data.shape[1]])
       if datasplitting[0:j]==[]:
           #.reshape(data.shape[0],data.shape[1]*k/(k-1))
           D_minus_j = np.array(datasplitting[j+1:k]).reshape(data.shape[0],(data.shape[1]*(k-1))//k)
       elif datasplitting[j+1:k]==[]:
           D_minus_j = np.array(datasplitting[0:j]).reshape(data.shape[0],(data.shape[1]*(k-1))//k)
       else:
           #print(datasplitting[0:j])
           #print('--------')
           #print(datasplitting[j+1:k])
           D_minus_j = np.concatenate((datasplitting[0:j],datasplitting[j+1:k]),axis=0).reshape(data.shape[0],(data.shape[1]*(k-1))//k)
       if labelssplitting[0:j]==[]:
           L_minus_j = np.array(labelssplitting[j+1:k]).reshape(labels.shape[0],(labels.shape[1]*(k-1))//k) 
       elif labelssplitting[j+1:k]==[]:
           L_minus_j = np.array(labelssplitting[0:j]).reshape(labels.shape[0],(labels.shape[1]*(k-1))//k)
       else:
           L_minus_j = np.concatenate((labelssplitting[0:j],labelssplitting[j+1:k]),axis=0).reshape(labels.shape[0],(labels.shape[1]*(k-1))//k)

       #L_minus_j = np.concatenate((labelssplitting[0:j-1],labelssplitting[j:labels.shape[1]]),axis=1)
       #print(datasplitting[j])
       #print(labelssplitting[j])
       #D_minus_j = np.delete(data,j,axis=1)
       #L_minus_j = np.delete(labels,j,axis=1)
       #print(data)
       #print(D_minus_j.shape)
       #print(L_minus_j.shape)
       #print(datasplitting[j].shape)
       #print(labelssplitting[j].shape)
       score = score + eval_classifier(learner,D_minus_j,L_minus_j,datasplitting[j],labelssplitting[j])
       #print(score)'''
