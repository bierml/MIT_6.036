...
def sgd(X, y, J, dJ, w0, step_size_fn, max_iter):
    ...
    for t in range(max_iter-1):
        ...
        ws.append(w.copy())
        fs.append(J(X[:,i].reshape((X.shape[0],1)),y[:,i].reshape((1,1)),w))
        w = w - step_size_fn(t)*dJ(X[:,i].reshape((X.shape[0],1)),y[:,i].reshape((1,1)),w)
    i = np.random.randint(X.shape[1])    
    ws.append(w.copy())
    fs.append(J(X[:,i].reshape((X.shape[0],1)),y[:,i].reshape((1,1)),w))
    return (w,fs,ws)
...
def sgdTest():
    ...
    def J(Xi, yi, w):
        ...
        #print(ridge_obj(Xi[:-1,:], yi, w[:-1,:], w[-1:,:], 0))
    ...
    def step_size_fn(t):
        return 10+t*(t-1)/1000
    #Insert code to call sgd on the above
    #print(X[:,1].reshape((X.shape[0],1)))
    #print(y[:,1].reshape((1,1)))
    #print(J(X[:,1].reshape((X.shape[0],1)),y[:,1].reshape((1,1)),np.zeros((X.shape[0],1))))
    print(sgd(X,y,J,dJ,np.zeros((X.shape[0],1)),step_size_fn,10))
    pass
